...
export class EventTypePageComponent implements OnInit {
  ...
  private loadEventType(id: number): void {
    ...
    this.eventTypesService.getEventTypeById(id).subscribe({
      next: (data) => {
        ...
        this.editData = { ...data };
        this.originalCategoryIds = data.recommendedProductCategories.map(
          (c) => c.id
        );
  ...
  loadProductCategories(): void {
    this.productCategoriesService
      .searchProductCategories(
        this.keywordEdit,
        this.currentPageEdit,
        this.itemsPerPageEdit
      )
      .subscribe((response) => {
  ...
  toggleCategorySelection(id: number): void {
    ...
    this.recommendedProductCategoriesList =
      this.recommendedProductCategoriesList.map((cat) => {
        if (cat.id === id) {
  ...
  updateEventType(): void {
    ...
    updatedEventType.recommendedProductCategories =
      updatedEventType.recommendedProductCategories.filter(
        (cat) => !this.removedProductCategories.includes(cat.id)
      );
  ...
  getActiveCategoriesCount(): number {
    if (!this.eventType?.recommendedProductCategories) {
      return 0;
    }
    return this.eventType.recommendedProductCategories.filter(
      (category) => category.status.name === 'ACTIVE'
    ).length;
  }
}
